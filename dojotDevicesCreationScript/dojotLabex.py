...
from utils.date_tools import (get_localized_datetime, get_localized_current_datetime, convert_from_string, get_timezone,
                              convert_timezone, convert_to_utc)
from exceptions.exception import (TemplateNotExists, TemplateAlreadyExists, DeviceNotExists, DeviceAlreadyExists,
        ...
        for value in values:
            ...
            try:
                history_dict[attr]["datetime"].append(
                    convert_timezone(
                        datetime_obj=convert_from_string(
                            datetime_str=value[time_key],
...
class Dojot(object):
    ...
    def request_token(self):
        ...
        if self.http_port is None:
            url_request_token = "http://" + self.ip + "/auth"
        else:
    ...
    def create_template(self, label, attrs):
        ...
        """
        if self.http_port is None:
            url_create_template = "http://" + self.ip + "/template"
        else:
    ...
    def get_all_templates_id(self):
        ...
        """
        if self.http_port is None:
            url_templates = "http://" + self.ip + "/template?page_size=999999&sortBy=label"
        else:
    ...
    def get_template(self, template_label):
        ...
        if template_exist:
            if self.http_port is None:
                url_template = "http://" + self.ip + "/template/" + str(template_id)
            else:
    ...
    def delete_template(self, label):
        ...
        if template_exists:
            if self.http_port is None:
                url_delete_template = "http://" + self.ip + "/template/" + str(template_id)
            else:
    ...
    def create_device(self, device_name, template_label, static_values=None):
        ...
        if template_exist:
            if self.http_port is None:
                url_create_device = "http://" + self.ip + "/device"
            else:
                url_create_device = "http://" + self.ip + ":" + str(self.http_port) + "/device"

            headers = {
                "Authorization": "Bearer {0}".format(self.jwt), "Content-Type": "application/json",
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:66.0) Gecko/20100101 Firefox/66.0",
                "Accept-Encoding": "*",
                "Connection": "keep-alive"
            }

            label = "{0}_{1}".format(template_label, device_name)
    ...
    def create_all_devices(self, devices=None):
        ...
        for template, devices_list in devices.items():
            for device in devices_list:
                device_name = "{0}_{1}".format(device["group_id"], device["db_id"])
                static = device["static_values"]
                try:
                    self.create_complete_device(device_name=device_name, template_label=template, static_values=static)
    ...
    def update_static(self, device_label, static_values):
        ...
        if self.http_port is None:
            url_update_static = "http://" + self.ip + "/device/" + device_id
        else:
    ...
    def get_all_devices_id(self):
        ...
        """
        if self.http_port is None:
            url_devices = "http://" + self.ip + "/device?page_size=999999&sortBy=label"
        else:
    ...
    def get_device(self, device_label):
        ...
        if device_exist:
            if self.http_port is None:
                url_device = "http://" + self.ip + "/device/" + device_id
            else:
                url_device = "http://" + self.ip + ":" + str(self.http_port) + "/device/" + device_id
            ...
            request = requests.get(url=url_device, headers=headers)
    ...
    def delete_device(self, device_label):
        ...
        if device_exist:
            if self.http_port is None:
                url_delete_device = "http://" + self.ip + "/device/" + str(device_id)
            else:
    ...
    def delete_all_devices(self, only_core_templates=True):
        """
        ...
        for dev_label, dev_id in device_id.items():
            template_label = self.get_device_template_label(device_label=dev_label)

            template_keys = list(self.templates.keys()) + [f"{k}_mensal" for k in self.templates.keys()]

            if only_core_templates:
                if template_label in template_keys:
                    try:
                        self.delete_device(device_label=dev_label)
    ...
    def get_history(self, device_id, params):
        ...
        """
        if self.http_port is None:
            url_device_history = "http://" + self.ip + "/history/device/" + device_id + "/history"
        else:
    ...
    def send_data_to_device(self, device_label, data, check_attrs=False):
        ...
        data_to_send = json.dumps(data, default=json_convert)  # Use default parameter to avoid the problem with numpy
        # int32 type. This problem exists when using data directly from VRM API.
